<aside class="ease-nav-brand z-990 fixed inset-y-0 my-4 block w-full max-w-64 -translate-x-full flex-wrap items-center justify-between overflow-y-hidden rounded-2xl border-0 bg-timberwolf p-0 antialiased shadow-xl transition-transform duration-200 dark:bg-el-salva dark:shadow-none xl:left-0 xl:ml-6 xl:translate-x-0" aria-expanded="false">
    <div class="h-19">
        <i class="ri-close-large-fill absolute right-0 top-0 cursor-pointer p-4 text-rose opacity-50 xl:hidden" sidenav-close></i>
        <a class="m-0 block whitespace-nowrap px-8 py-6 text-sm text-reno-sand dark:text-peach-yellow" href="{{ route("dashboard") }}">
            <img src="{{ asset("img/logo.png") }}" class="ease-nav-brand inline w-16 rounded-lg transition-all duration-200 dark:hidden" alt="main_logo" />
            <img src="{{ asset("img/logo.png") }}" class="ease-nav-brand hidden w-16 rounded-lg transition-all duration-200 dark:inline" alt="main_logo" />
            <span class="ease-nav-brand ml-2 font-bold transition-all duration-200 font-spaceGrotesk">Laravel MOORA</span>
        </a>
    </div>
    ...
    <div class="h-sidenav block max-h-screen w-auto grow basis-full items-center overflow-auto">
        <ul class="mb-0 flex flex-col pl-0">
            ...
            <li class="mt-4 w-full">
                <h6 class="ml-2 pl-6 text-xs font-bold uppercase leading-tight text-el-salva dark:text-peach-yellow opacity-60">Pengaturan</h6>
            </li>

            <li class="mt-0.5 w-full">
                <form method="POST" action="{{ route("logout") }}" enctype="multipart/form-data">
                    @csrf
                    <div class="py-2.7 ease-nav-brand mx-2 my-0 flex items-center whitespace-nowrap px-4 text-sm transition-colors hover:rounded-lg hover:bg-el-salva/10 dark:hover:bg-peach-yellow/30 dark:text-white">
                        <div class="mr-2 flex h-8 w-8 items-center justify-center rounded-lg bg-center stroke-0 text-center xl:p-2.5">
